// ------------------------------------------------------------------
// io::mac_input - stardazed
// (c) 2016 by Arthur Langereis
// ------------------------------------------------------------------

#include "io/input.hpp"
#include "io/mac_VKeyCodes.hpp"
...
static std::array<Key, 512> keyTransTable_s {};

static void buildKeyTranslationTable() {
	std::fill(keyTransTable_s.begin(), keyTransTable_s.end(), Key::None);
	auto& ktt = keyTransTable_s;
		...
		do {
			...
			if (ev) {
				...
				switch(eventType) {
					case NSKeyDown:
					case NSKeyUp:
						if (keyTransTable_s[kVK_ANSI_A] == Key::None) {
							buildKeyTranslationTable();
						}
						if (eventType == NSKeyDown) {
							keyboard.keyDownEvent(keyTransTable_s[ev.keyCode]);
						}
						else {
							keyboard.keyUpEvent(keyTransTable_s[ev.keyCode]);
						}
						break;
